...
async def on_message(message):
    ...
    # Si el bot no está activo, ignorar
    if not bot_state["active"]:
        return
    
    # Verificar si el mensaje es exactamente una letra
    content = message.content.strip()
    if len(content) == 1 and content.isalpha():
        letter = content.upper()
        
        # Verificar si la letra ya fue dicha
...
async def turn_on(interaction: discord.Interaction):
    if bot_state["active"]:
        # Ya está encendido, solo resetear
        bot_state["letters"] = {}
        ...
        # Está apagado, encender y resetear
